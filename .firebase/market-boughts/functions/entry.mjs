...
import {
  appendForwardSlash as appendForwardSlash$1,
  joinPaths,
  slash,
  prependForwardSlash,
...
import {
  ...
  d as dateTimeFormat,
  A as AstroCookies,
  c as computePreferredLocale,
  a as computePreferredLocaleList,
  b as computeCurrentLocale,
  r as routeIsRedirect,
  e as redirectRouteStatus,
  f as redirectRouteGenerate,
  g as routeIsFallback,
  h as attachCookiesToResponse,
  i as createAPIContext,
  j as callEndpoint,
  k as callMiddleware,
  L as Logger,
  m as AstroIntegrationLogger,
  R as RouteCache,
  n as getSetCookiesFromResponse,
  o as createRenderContext,
  manifest,
} from "./manifest_732c88c5.mjs";
import { yellow, dim, bold, cyan, red, reset } from "kleur/colors";
import {
  A as AstroError,
  R as ReservedSlotName,
  j as renderSlotToString,
  k as renderJSX,
  l as chunkToString,
...
function createI18nMiddleware(i18n, base, trailingSlash) {
  ...
  return async (context, next) => {
    ...
    if (response instanceof Response) {
      ...
      const pathnameContainsDefaultLocale = url.pathname.includes(
        `/${defaultLocale}`,
      );
        ...
        if (url.pathname === base + "/" || url.pathname === base) {
          if (trailingSlash === "always") {
            return context.redirect(
              `${appendForwardSlash$1(joinPaths(base, i18n.defaultLocale))}`,
            );
          } else {
...
async function renderPage({ mod, renderContext, env, cookies }) {
  if (routeIsRedirect(renderContext.route)) {
    return new Response(null, {
      status: redirectRouteStatus(
        renderContext.route,
        renderContext.request.method,
      ),
      headers: {
        location: redirectRouteGenerate(
          renderContext.route,
  ...
  if (!Component)
    throw new Error(
      `Expected an exported Astro component but received typeof ${typeof Component}`,
    );
  ...
  if (
    mod.frontmatter &&
    typeof mod.frontmatter === "object" &&
    "draft" in mod.frontmatter
  ) {
    env.logger.warn(
      "astro",
...
class Pipeline {
  ...
  async #tryRenderRoute(renderContext, env, mod, onRequest) {
    ...
    switch (renderContext.route.type) {
      ...
      case "redirect": {
        if (onRequest) {
          return await callMiddleware(env.logger, onRequest, apiContext, () => {
            return renderPage({
              mod,
              renderContext,
              env,
      ...
      default:
        throw new Error(
          `Couldn't find route of type [${renderContext.route.type}]`,
        );
    }
  }
  /**
  ...
  async #renderError(
    ...
    if (errorRouteData) {
      ...
      try {
        ...
        const response2 = await this.#pipeline.renderRoute(
          newRenderContext,
          page,
        );
...
function createRequestFromNodeRequest(req, options) {
  ...
  let bodyProps = {};
  const bodyAllowed =
    method !== "HEAD" && method !== "GET" && !options?.emptyBody;
  if (bodyAllowed) {
  ...
  Object.values(os.networkInterfaces())
    ...
    .filter(
      (detail) =>
        detail &&
        detail.address &&
        (detail.family === "IPv4" || // @ts-expect-error Node 18.0 - 18.3 returns number
          detail.family === 4),
    )
    .forEach((detail) => {
      let host = detail.address.replace(
        "127.0.0.1",
        hostname === void 0 || wildcardHosts.has(hostname)
          ? "localhost"
          : hostname,
      );
      if (host.includes(":")) {
...
function startServer$1(app, options) {
  ...
  const host = getResolvedHostForHttpServer(
    process.env.HOST !== void 0 && process.env.HOST !== ""
      ? process.env.HOST
      : options.host,
  );
...
const adapter = /*#__PURE__*/ Object.freeze(
...
const _page0 = () => import("./chunks/node_56b1aec0.mjs");
const _page1 = () => import("./chunks/index_99eee715.mjs");
const _page2 = () => import("./chunks/dashboard_5fcc9e21.mjs");
const _page3 = () => import("./chunks/register_fc511bbd.mjs");
const _page4 = () => import("./chunks/signin_2e492440.mjs");
const _page5 = () => import("./chunks/register_f8cf29b6.mjs");
const _page6 = () => import("./chunks/signout_e4cc2d55.mjs");
const _page7 = () => import("./chunks/signin_6b4b92da.mjs");
...
const _args = {
  mode: "middleware",
  client: "file:///Users/f.moreno/Projects/market-boughts/dist/client/",
  server: "file:///Users/f.moreno/Projects/market-boughts/dist/server/",
  host: false,
  port: 4321,
  assets: "_astro",
};
